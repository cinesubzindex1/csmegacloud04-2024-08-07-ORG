...
const { encryptData, generateKey } = require('../utils/crypto');
...
async function generateLinkData(file_id,filename) {
    const encrypted_id = await encryptString(file_id);
    const expiry = Date.now() + 1000 * 60 * 60 * 24 * config.file_link_expiry;
    const encrypted_expiry = await encryptString(expiry.toString());
    const integrity = await genIntegrity(`${file_id}|${expiry}`);
    const key = generateKey()
    const tag = encryptData(file_id,key)
    const fake = {
        file : generateKey(encrypted_id.length),
        expiry : generateKey(encrypted_expiry.length),
    }
    const url = `/download.csdl/${encodeURIComponent(filename)}?file=${encodeURIComponent(encrypted_id)}&expiry=${encodeURIComponent(encrypted_expiry)}&mac=${encodeURIComponent(integrity)}&inline=true`;
    return {url,expiry:encodeURIComponent(encrypted_expiry), mac : encodeURIComponent(integrity),id:encodeURIComponent(encrypted_id),filename:encodeURIComponent(filename),tag,iv : key.split('').reverse(),fake,path:`/download.csdl/${encodeURIComponent(filename)}?`};
}

module.exports = {download,getFileInfo,DeleteFile,generateLink,generateLinkData}